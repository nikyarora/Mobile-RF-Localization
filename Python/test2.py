...
ser = serial.Serial(2, 115200, timeout=0)    #port 0, baudrate 19200, no timeout
i=0
#setting up screen
wn = turtle.Screen() 
...
while(ser.isOpen()):
    ...
    if(len(C)>0):
        ...
        r1_dB= convert_to_signed(C[1]) +45     # radius in dB
        r2_dB= convert_to_signed(C[3]) +45
        r3_dB= convert_to_signed(C[5]) +45
