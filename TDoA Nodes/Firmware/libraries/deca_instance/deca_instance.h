...
#define STANDARD_FRAME_SIZE         66 // originally 127, min possible 55
...
#define BLINK_SLEEP_DELAY					100 //ms
#define POLL_SLEEP_DELAY					100 //ms
...
// This value may need to be 205 to comply with above 
#define IDLE_TIME						(25) //this time is added to the FIXED_RP_REPLY_DELAY_US to meet the above requirement
...
#define FIXED_DP_REPLY_DELAY_US         200 //us  //response delay time before sending the Ranging Init
...
#define FIXED_LONG_BLINK_RESPONSE_DELAY       (2*FIXED_REPLY_DELAY) //NOTE: this should be a multiple of FIXED_LONG_REPLY_DELAY see DELAY_MULTIPLE below
...
typedef struct
{
    ...
    int newrange;
    uint16_t newrangeancaddress;
    uint16_t newrangetagaddress;
...
void instancesetantennadelays(float fdelay) ;                      // delay in nanoseconds
...
void instance_startmeasure();  // Force start of measure
...
uint32_t instance_get_idisti(void);
...
#define DWT_PRF_64M_RFDLY   (515.462f)
#define DWT_PRF_16M_RFDLY   (514.9067f)
