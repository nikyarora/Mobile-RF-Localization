...
int destaddress(instance_data_t *inst)
{
    ...
    while(getnext && inst->anchorListIndex <= inst->payload.anchorListSize)
    {
        if(((uint64_t)0x1 << inst->anchorListIndex) & inst->payload.anchorPollMask)
        {
            uint64_t addr = 0xDECA020000000000;
            addr |= inst->anchorListIndex;
            inst->currentAnchorIndex = inst->anchorListIndex;
            memcpy(inst->msg.destAddr, &addr, ADDR_BYTE_SIZE_L);
            getnext = 0;
        }
    ...
    while(tempanchorListIndex <= inst->payload.anchorListSize)
        ...
        if(((uint64_t)0x1 << tempanchorListIndex) & inst->payload.anchorPollMask)
        {
            return 0;
        }
        tempanchorListIndex++;
...
void setup_DW1000RSTnIRQ(int i)
{
    if (i)
...
float calcRxPowerf(uint16 aN, uint16 aC)
{
    float N = aN;
    float C = aC;
    float v = C*0x20000;  // C*2^17
    v/=N*N;
    v = 10.0*log10f(v) - 115.72; // 115.72 for 16MHz, 121.74 for 64MHz prf
    return v;
}

uint32 ulog2 (uint32 u)
{
    uint32 s, t;
...
int16 calcRxPower(uint16 N, uint16 C)
{
    uint32 v = C*0x20000;  // C*2^17
    uint32 N2 = N;
    N2 = N2*N2;
    v/=N2;

    v = 3*ulog2(v) - 116; // 115.72 for 16MHz, 121.74 for 64MHz prf
    return v;
}
        ...
        case TA_INIT :
            switch (inst->mode)
            {
                case TAG:
                {
                    ...
                    memcpy(inst->eui64, &inst->payload.tagAddress, sizeof(uint64));
                ...
                case ANCHOR:
                {
                    memcpy(inst->eui64, &inst->payload.anchorAddress, sizeof(uint64));
                    dwt_seteui(inst->eui64);
                    dwt_setpanid(inst->panid);
                    
                    // Anchor 00 will receive all reports
                    if (inst->eui64[0] == 0x00)
                    {
                    ...
                    dwt_setdblrxbuffmode(DOUBLE_RX_BUFFER); //enable double RX buffer
        ...
        case TA_TXPOLL_WAIT_SEND :
            {
                ...
                //spi_debugi(inst->anchorDistanceIndex);
#endif
        ...
        case TA_RX_WAIT_DATA :                                                                     // Wait RX data
            switch (message)
            {
                ...
                case DWT_SIG_RX_OKAY :
                {
                    ...
                    // Extract power of received signal
                    if (dw_event->diag_N != 0)
                    {
                        inst->rxPwr = calcRxPower(dw_event->diag_N, dw_event->diag_C);
                    }
					...
					switch(dw_event->msgu.frame[1])
					{
						...
						case 0xC8: //
							...
							memcpy(&dstAddr[0], &(dw_event->msgu.rxmsg_sl.destAddr[0]), ADDR_BYTE_SIZE_L);
						...
						case 0x88: //
							...
							memcpy(&dstAddr[0], &(dw_event->msgu.rxmsg_ss.destAddr[0]), ADDR_BYTE_SIZE_L);
                    ...
                    else
                    {
                        ...
                        switch(fcode)
                        {
                            ...
                            case RTLS_DEMO_MSG_TAG_POLL:
                            {
								...
								if((dstAddr[0] != inst->eui64[0]) || (dstAddr[1] != inst->eui64[1])) //if not addressed to us
								{
									inst->testAppState = TA_RXE_WAIT ;              // wait for next frame
									break;
								}
                                ...
                                instcleartaglist();
                                instaddtagtolist(inst, &(srcAddr[0]));
...
#if (USING_64BIT_ADDR == 1)
								memcpy(&inst->replyAddress[0], &srcAddr[0], ADDR_BYTE_SIZE_L); //remember who to send the reply to (set destination address)
#else
								memcpy(&inst->replyAddress[0], &srcAddr[0], ADDR_BYTE_SIZE_S); //remember who to send the reply to (set destination address)
#endif
...
// -------------------------------------------------------------------------------------------------------------------
#if NUM_INST != 1
#warning These functions assume one instance only
#endif
...
int instance_init_s(int mode)
{
    ...
    instance_data[instance].mode =  (INST_MODE)mode;                                // assume anchor,
    ...
    //this is platform dependent - only program if DW EVK/EVB
    dwt_setleds(1) ; //configure the GPIOs which control the LEDs on EVBs
...
void instancesetreplydelay(int delayms, int datalength) //delay in ms
{
    ...
    {
#if (USING_64BIT_ADDR == 1)
		msgdataleni = TAG_FINAL_MSG_LEN + FRAME_CRTL_AND_ADDRESS_L + FRAME_CRC;
#else
		msgdataleni = TAG_FINAL_MSG_LEN + FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC;
#endif
		//msgdatalen += instance_data[instance].payload.payloadLen ;
    }
	else //blink message
    {
		msgdataleni = datalength;
    }

	int xi = (msgdataleni*8)/330+1;
    ...
    {
    case DWT_PLEN_4096 : preambleleni = 4096; break;
    case DWT_PLEN_2048 : preambleleni = 2048; break;
    case DWT_PLEN_1536 : preambleleni = 1536; break;
    case DWT_PLEN_1024 : preambleleni = 1024; break;
    case DWT_PLEN_512  : preambleleni = 512; break;
    case DWT_PLEN_256  : preambleleni = 256; break;
    case DWT_PLEN_128  : preambleleni = 128; break;
    case DWT_PLEN_64   : preambleleni = 64; break;
    }
    
	//preamble  = plen * (994 or 1018) depending on 16 or 64 PRF
	if(instance_data[instance].configData.prf == DWT_PRF_16M)
	{
		preambleleni = ((sfdlen + preambleleni) * 994)/1000;
    }
    else
    {
		preambleleni = ((sfdlen + preambleleni) * 1018)/1000;
    }
	...
	if(datalength != 0)
    {
		...
		instance_data[instance].fwtoTimeB_sy = 16 + ((preambleleni + msgdataleni/1000)*975)/1000 + margin;
		instance_data[instance].rnginitW4Rdelay_sy = (int)(delayms * 975) - 16 - ((preambleleni + (msgdataleni/1000))*975)/1000;
    }
    else
    {
		//set the frame wait timeout time - total time the frame takes in symbols
		instance_data[instance].fwtoTime_sy = 16 + ((preambleleni + msgdataleni/1000)*975)/1000 + margin;
        
		//this is the delay used for the delayed transmit (when sending the ranging init, response, and final messages)
		instance_data[instance].fixedReplyDelay = delayms; // 638976000DWT
        instance_data[instance].fixedReplyDelay*= 63897600;
        ...
        instance_data[instance].fixedReplyDelay_sy = (delayms * 975) - 16 - ((preambleleni + (msgdataleni/1000))*975)/1000; 
        //subtract 16 symbols, as receiver has a 16 symbol start up time
    }
}
...
void instancesetaddresses(instanceAddressConfig_t *plconfig)
    ...
    memcpy(instance_data[instance].eui64, &(plconfig->tagAddress), sizeof(uint64));
}
#endif
...
uint64 instance_get_addr(void) //get own address
{
    int instance = 0;
    uint64 x = 0;
    memcpy(&x, instance_data[instance].eui64, sizeof(uint64));
    return (x);
}

uint16_t instance_get_newrangeancaddress()
{
    return instance_data[0].newrangeancaddress;
}

uint16_t instance_get_newrangetagaddress()
{
...
uint64 instance_get_anchaddr(void) //get anchor address (that sent the ToF)
{
    int instance = 0;
    uint64 x = 0;
    memcpy(&x, instance_data[instance].replyAddress, sizeof(uint64));
    return (x);
}

int16 instance_get_rxpwr(void)
{
    return instance_data[0].rxPwr;
}
...
int instance_getstate()
{
    return (int)instance_data[0].testAppState;
}
