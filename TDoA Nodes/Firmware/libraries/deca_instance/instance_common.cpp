...
void reportTOFi(instance_data_t *inst)
{
        int32_t distance ;
        int64 tofi ;
        ...
        // convert to seconds 
        distance = (tofi*1000000)/852814; // 1.0/499.2e6/128.0*0.25*299702547.0

        if ((distance < 0) || (distance > 20000000))    // discard any results less than <50 cm or >20km
            return;
        inst_idisti = distance;
...
// Version without otp usage
void instance_config_nootp(instanceConfig_t *config)
{
    int instance = 0 ;
    int use_otpdata = DWT_LOADANTDLY | DWT_LOADXTALTRIM;
    uint32 power = 0;
    ...
    //configure the channel parameters
    dwt_configure_nootp(&instance_data[instance].configData) ;
    //dwt_configure(&instance_data[instance].configData, use_otpdata) ;

    instance_data[instance].configTX.PGdly = 0xc0;
    power = 0x0E08281F;
...
void instancesetantennadelays(float fdelay)
{
    // 4042
    int instance = 0 ;
    instance_data[instance].txantennaDelay = (fdelay/ 2.0) * 1e-9 / DWT_TIME_UNITS;
    // -------------------------------------------------------------------------------------------------------------------
    // set the antenna delay, we assume that the RX is the same as TX.
    dwt_setrxantennadelay(instance_data[instance].txantennaDelay);
    dwt_settxantennadelay(instance_data[instance].txantennaDelay);
...
void instancesetantennadelays_i(uint16 delay)
{
    // 0x4042
    instance_data[0].txantennaDelay = delay;
    // -------------------------------------------------------------------------------------------------------------------
    // set the antenna delay, we assume that the RX is the same as TX.
    dwt_setrxantennadelay(instance_data[0].txantennaDelay);
...
uint32_t instance_get_idisti(void) //get instantaneous range
{
    uint32_t x = inst_idisti;
...
void instance_rxcallback(const dwt_callback_data_t *rxd)
{
    ...
    dw_event.diag_N = diag.rxPreamCount;
    dw_event.diag_C = diag.maxGrowthCIR;
...
void instance_putevent(event_data_t newevent)
{
	...
	newevent.type = 0;
	//newevent.eventtime = spi_getTickCount();
...
event_data_t* instance_getevent(int x)
{
	...
	dw_event_g.diag_C = instance_data[instance].dwevent[indexOut].diag_C ;
	dw_event_g.diag_N = instance_data[instance].dwevent[indexOut].diag_N ;
...
int instance_run(void)
{
    ...
    {
        if(instance_data[instance].mode == TAG && instance_data[instance].tagSleepTime_ms > 0) //Tag (is either in RX or sleeping)
        {
            instance_data[instance].instancetimer = spi_getTickCount() + instance_data[instance].tagSleepTime_ms; //set timeout time
    ...
    {
        if(instance_data[instance].instancetimer < spi_getTickCount())
        {
			event_data_t dw_event;
            instance_data[instance].instancetimer_en = 0;
			dw_event.rxLength = 0;
            ...
            //spi_debug("PC timeout DWT_SIG_RX_TIMEOUT");
			instance_putevent(dw_event);
        }
    }
...
void instance_startmeasure()
{
    event_data_t dw_event;
    instance_data[0].instancetimer_en = 0;
    dw_event.rxLength = 0;
    dw_event.type = DWT_SIG_RX_TIMEOUT;
    dw_event.type2 = 0x80 | DWT_SIG_RX_TIMEOUT;
    //printf("PC timeout DWT_SIG_RX_TIMEOUT\n");
