...
const tx_struct TABLE_MEM_TYPE txSpectrumConfig[8] =
{
...
const uint16 TABLE_MEM_TYPE rfDelays[2] = {
...
int powertest(void)
{
	...
	{
		uint32 pow = txSpectrumConfig[configData.chan].txPwr[configData.prf - DWT_PRF_16M] & 0xFF ;
...
void xtalcalibration(void)
{
    ...
    uint32 pow = txSpectrumConfig[chan].txPwr[prf - DWT_PRF_16M] & 0xFF ;
	configTx.power = (pow | (pow << 8) | (pow << 16) | (pow << 24));
