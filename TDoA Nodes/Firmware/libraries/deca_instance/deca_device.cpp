...
#else
    uint16 reg16 = (uint16)pgm_read_word_near(lde_replicaCoeff+config->rxCode);
#endif
...
//    configure PLL2/RF PLL block CFG
    ...
    dwt_write32bitoffsetreg(RF_CONF_ID, RF_TXCTRL_OFFSET, 
        pgm_read_dword_near(tx_config + chan_idx[chan]));
#endif
    ...
    //DTUNE2
#ifndef DWT_STORE_TABLES_IN_PROGMEM
    uint32 reg32 = digital_bb_config[prfIndex][config->rxPAC];
#else
    uint32 reg32 = pgm_read_dword_near(((prfIndex) ? digital_bb_config_1 : digital_bb_config_0) + config->rxPAC);
#endif
    dwt_write32bitoffsetreg(DRX_CONF_ID, DRX_TUNE2_OFFSET, reg32);
    ...
    {
        uint32 x = ((uint32)config->txPreambLength | (uint32)config->prf)  <<  16;
 ...
 * returns DWT_DECA_SUCCESS for success, or DWT_DECA_ERROR for error
 */
int dwt_configure_nootp(dwt_config_t *config)
{
    uint8 nsSfd_result  = 0;
    uint8 useDWnsSFD = 0;
    uint8 chan = config->chan ;
...
// Version allowing double buffering
void dwt_isr_dblbuff(void) // assume interrupt can supply context
{
    uint32  status = 0;
    uint32  clear = 0; // will clear any events seen
